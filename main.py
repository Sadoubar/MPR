...
st.set_page_config(
    page_title="Calculateur MaPrimeRénov' Ampleur",
    page_icon="✨",
    layout="wide",
...
st.markdown("""<style>
    ...
    /* Polices */
    body, .stButton>button, .stTextInput>div>div>input, .stNumberInput>div>div>input, .stSelectbox>div>div, .stRadio>div>label>div {
    ...
    /* Autres */
    .stButton>button { border-radius: 6px; padding: 0.6rem 1.5rem; font-weight: 600; font-size: 1rem; }
    ...
    div[data-testid="stExpander"] details { border: 1px solid #D1D5DB; border-radius: 8px; box-shadow: none;}
    div[data-testid="stExpander"] summary { font-weight: 600; color: #1E3A8A;}
...
def creer_graphique_financement(aide_mpr, reste_charge, ttc, aide_mar=0):
    ...
    fig.update_layout(
        title={'text': "Répartition du Financement", 'y':0.95, 'x':0.5, 'xanchor': 'center', 'yanchor': 'top', 'font_size': 18, 'font_color': '#111827'},
        height=360,
...
with col_h1:
    st.markdown("<h1 class='main-title'>✨ Simulateur MaPrimeRénov' Ampleur TEST ✨</h1>", unsafe_allow_html=True)
    st.markdown("<p class='sub-header'>Estimez vos aides pour une rénovation énergétique performante</p>", unsafe_allow_html=True)
with col_h2:
    st.markdown("""
    <div class='profile-section' style='margin-top:1rem;'>
        <img src='https://st3.depositphotos.com/1026550/15275/i/450/depositphotos_152750910-stock-photo-environment-conservation-concept.jpg' class='profile-image' alt='Profil'>
        <div class='profile-text'>
            <strong>Sadou BARRY</strong><br>
            Passionné d’Éco-Transition & d’Efficacité Énergétique<br>
            <a href='https://www.linkedin.com/in/sadou-barry-881868164/' target='_blank'>Contactez-moi sur LinkedIn</a>
        </div>
    </div>
...
with tab1:
    st.markdown("<h2 class='section-title'>1. Entrez vos informations</h2>", unsafe_allow_html=True)

    # Container pour les inputs
    with st.container():
        col1, col2, col3 = st.columns(3)
        with col1:
            st.markdown("<h5>👤 Ménage <span style='color:red;'>*</span></h5>", unsafe_allow_html=True)
            code_postal = st.text_input("Code postal", max_chars=5, key="cp", help="Code postal du logement (5 chiffres)")
            personnes = st.number_input("Personnes foyer", min_value=1, value=2, step=1, key="pers", help="Nombre de personnes composant le foyer fiscal")
            revenu_fiscal = st.number_input("Revenu fiscal réf. (€)", min_value=0.0, value=30000.0, step=100.0, format="%.0f", key="rev", help="Votre revenu fiscal de référence (avis d'imposition N-1)")
        with col2:
            st.markdown("<h5>🛠️ Projet <span style='color:red;'>*</span></h5>", unsafe_allow_html=True)
            cout_travaux_ht = st.number_input("Coût travaux HT (€)", min_value=1.0, value=50000.0, step=100.0, format="%.0f", key="ht", help="Coût total HT des travaux éligibles (doit être > 0)")
            cout_travaux_ttc_defaut = round(cout_travaux_ht * 1.055, 0) if cout_travaux_ht > 0 else 0.0
            cout_travaux_ttc = st.number_input("Coût travaux TTC (€)", min_value=1.0, value=cout_travaux_ttc_defaut, step=100.0, format="%.0f", key="ttc", help="Coût total TTC (vérifiez la TVA applicable, souvent 5.5%)")
            inclure_mar = st.checkbox("Inclure Mon Accompagnateur Rénov'", value=True, key="mar", help="Inclure l'aide pour l'accompagnement obligatoire")
            habitat_indigne = st.checkbox("Cas d'habitat indigne", value=False, key="indigne", help="Cocher si traitement d'habitat indigne inclus (plafond MAR à 4000€)")
        with col3:
            st.markdown("<h5>⚡ Performance <span style='color:red;'>*</span></h5>", unsafe_allow_html=True)
            classe_actuelle = st.selectbox("Classe actuelle", cf.CLASSES_ORDRE, index=cf.CLASSES_ORDRE.index('G'), key="cl_act", help="Classe DPE avant travaux")
            classe_apres = st.selectbox("Classe visée", cf.CLASSES_ORDRE, index=cf.CLASSES_ORDRE.index('B'), key="cl_apr", help="Classe DPE visée après travaux")
            gain_options_map = {"2 classes": 2, "3 classes": 3, "4 classes ou plus": 4}
            gain_choisi_str = st.radio(
                "Gain de classes visé pour le calcul",
                options=gain_options_map.keys(),
                index=1, key="gain_choisi", horizontal=True,
                help="Sélectionnez le saut de classe pour déterminer le taux/plafond de l'aide."
            )
            gain_choisi_par_user = gain_options_map[gain_choisi_str]
            renovation_deux_etapes = st.checkbox("Rénovation en 2 étapes", value=False, key="deux_etapes", help="Si le projet est réalisé sur 5 ans (concerne G, F, E initialement)")
    ...
    if region is None:
        ...
        messages_validation.append(("warning", "Code postal invalide."))
    if not isinstance(personnes, int) or personnes < 1:
        calcul_possible = False
        messages_validation.append(("warning", "Nombre de personnes invalide."))
    if not isinstance(revenu_fiscal, (int, float)) or revenu_fiscal < 0:
        calcul_possible = False
        messages_validation.append(("warning", "Revenu fiscal invalide."))
    if not isinstance(cout_travaux_ht, (int, float)) or cout_travaux_ht <= 0:
        calcul_possible = False
        messages_validation.append(("warning", "Coût HT invalide."))
    if not isinstance(cout_travaux_ttc, (int, float)) or cout_travaux_ttc <= 0:
        calcul_possible = False
        messages_validation.append(("warning", "Coût TTC invalide."))
    elif cout_travaux_ttc < cout_travaux_ht:
        calcul_possible = False
        messages_validation.append(("warning", "Coût TTC < Coût HT."))
    ...
    if calcul_possible:
        ...
        if not perf_ok:
            ...
            validation_placeholder.info(
                f"Gain réel calculé : {gain_reel} classes ({classe_actuelle} → {classe_apres}). "
                f"Le calcul de l'aide se basera sur votre choix de **{gain_choisi_par_user} classes**."
            )
            if gain_reel < gain_choisi_par_user:
                messages_validation.append((
                    "warning",
                    f"Attention: Le gain réel ({gain_reel}) est inférieur au gain choisi ({gain_choisi_par_user}). "
                    "Le projet doit atteindre ce gain choisi pour l'aide correspondante."
                ))

            est_passoire = classe_actuelle in ["F", "G"]
            index_apres = cf.CLASSES_ORDRE_INV.get(classe_apres, -1)
            sortie_passoire_atteinte = est_passoire and index_apres >= cf.CLASSES_ORDRE_INV["D"]
            ...
            if renovation_deux_etapes and classe_actuelle not in ["G", "F", "E"]:
                messages_validation.append(("info", "Info: Rénovation en 2 étapes concerne surtout G/F/E."))
    ...
    with col_btn2:
        calcul_btn = st.button(
            "🚀 Lancer l'estimation de l'aide",
            use_container_width=True,
            disabled=not calcul_possible,
            type="primary"
        )

    # Affichage des résultats
    if calcul_btn:
        categorie, categorie_display, region, seuils_perso = cf.determiner_categorie(
            code_postal, personnes, revenu_fiscal
        )
        ...
        if categorie is None:
            ...
            resultats = cf.calculer_aide_maprimereno(
                categorie=categorie,
                gain_reel=gain_choisi_par_user,  # Correction ici - gain_choisi remplacé par gain_reel
            ...
            if resultats is None:
                afficher_alerte("Erreur interne calcul.", "error")
            else:
                ...
                with res_col1:
                    ...
                    classe_act_fmt = formatter_classe(classe_actuelle)
                    classe_apr_fmt = formatter_classe(classe_apres)
                    ...
                    gain_utilise_msg = f"Calcul basé sur gain choisi: {gain_choisi_par_user} cl."

                    st.markdown(f"""
                    <div class='info-alert-box' style='margin-top: 15px;'>
                        <strong>Contexte :</strong><br>
                        - Profil: {categorie_display} ({region_nom})<br>
                        - Performance: {classe_act_fmt} → {classe_apr_fmt} (Gain réel: {gain_reel} cl.){bonus_msg_res}<br>
                        - {gain_utilise_msg}<br>
                ...
                with st.expander("🔍 Voir les détails du calcul"):
                    ...
                    with detail_col1:
                        ...
                        st.markdown(f"- **Gain choisi pour calcul : {gain_choisi_par_user} classes**")
                        st.markdown(f"- Gain réel constaté : {gain_reel} classes")
                    ...
                    with detail_col2:
                        ...
                        if seuils_perso and len(seuils_perso) == 3:
                            ...
                            data_seuils = {
                                "Catégorie": ["Très modestes", "Modestes", "Intermédiaires", "Supérieurs"],
...
with tab3:
    ...
    with faq_col1:
        with st.expander("MPR Ampleur, c'est quoi ?"):
            st.markdown("L'aide pour les rénovations globales avec ≥ 2 classes de gain et accompagnement obligatoire.")
        with st.expander("Comment est calculé le gain ?"):
            st.markdown("Différence entre classe avant et après travaux (ex: F -> C = 3 classes). Calculé sur la base d'un audit énergétique.")
        with st.expander("Qui est Mon Accompagnateur Rénov' ?"):
            st.markdown("Professionnel agréé obligatoire qui aide de l'audit au suivi des travaux.")
    with faq_col2:
        with st.expander("L'écrêtement, c'est quoi ?"):
            st.markdown("Plafond de l'aide basé sur un % du coût TTC des travaux (variable selon les revenus).")
        with st.expander("Cumul possible avec d'autres aides ?"):
            st.markdown("Oui avec Éco-PTZ, aides locales, TVA 5.5%. Non cumulable avec les CEE classiques ou MPR par geste.")
        with st.expander("Rénovation en 2 étapes ?"):
            st.markdown("Pour G/F/E sur 5 ans max. Atteindre C (si G/F) ou B (si E) minimum à la fin. Voir MAR pour détails.")
